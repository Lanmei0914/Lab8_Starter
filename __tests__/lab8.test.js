describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let cartCountValue = await innerText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      //console.log(`Checking in cart on screen after reload item ${i + 1}/${prodItems.length}`);
      let shadowRoot = await prodItems[i].getProperty('shadowRoot');
      let button = await shadowRoot.$('button');
      let innerText = await button.getProperty('innerText');
      let textValue = await innerText.jsonValue();
      if (textValue != 'Remove from Cart') { allButtonsRemove = false; }
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let innerText = await button.getProperty('innerText');
      let textValue = await innerText.jsonValue();
      if (textValue != 'Add to Cart') { removed = false; }
    }
